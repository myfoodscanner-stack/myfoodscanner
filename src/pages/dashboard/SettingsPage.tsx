...
export const SettingsPage: React.FC<SettingsPageProps> = ({ onNavigate }) => {
  ...
  // Live timer calculation for 48h guarantee
  const [remainingTimeStr, setRemainingTimeStr] = useState<string>('');
  const [isEligible48h, setIsEligible48h] = useState(false);
  ...
  useEffect(() => {
    const updateTimer = () => {
      ...
      const remainingMs = total48hMs - elapsedMs;

      if (remainingMs > 0) {
        setIsEligible48h(true);
        ...
        setIsEligible48h(false);
        setRemainingTimeStr('Guarantee window expired');
      }
    };
  ...
  const handleClaimRefund = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch('/api/subscription/refund-48h', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
      });

      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Refund processing failed.');
    ...
    <div className="min-h-screen bg-[#0d160d] text-emerald-100 py-6 px-4 pb-24 md:pb-12">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-[#142314] border border-[#263e26] rounded-3xl p-6 space-y-6">
          ...
          {/* 48-Hour Money-Back Guarantee Real-time Widget */}
          {user?.subscription_status === 'active' && (
            <div className="bg-gradient-to-r from-[#173017] to-[#122412] border border-emerald-500/40 rounded-2xl p-5 space-y-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-200/70">
                      100% full refund in 1 click without any questions asked.
                    </p>
                  </div>
                </div>
                ...
                {isEligible48h && (
                  <div className="bg-black/40 px-3 py-1.5 rounded-xl border border-emerald-500/30 text-right">
                    <span className="text-[10px] text-emerald-400/80 block uppercase font-bold">Time Left</span>
              ...
              {isEligible48h ? (
                <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3">
                  <p className="text-[11px] text-emerald-300/80">
                    You are currently within your 48-hour satisfaction guarantee window.
                  </p>
                  <button
                    onClick={() => setShowRefundModal(true)}
                    className="w-full sm:w-auto px-4 py-2 rounded-xl bg-red-950/80 hover:bg-red-900 border border-red-500/50 text-red-200 font-bold text-xs flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                  >
                ...
                <p className="text-[11px] text-emerald-400/60 italic">
                  48-hour guarantee window has concluded. Your subscription will renew automatically.
                </p>
              )}

              <p className="text-[10px] text-emerald-300/50 pt-1 border-t border-emerald-500/20">
                *Valid strictly on your 1st initial payment within 48 hours. Subsequent renewal payments are non-refundable.
              </p>
            </div>
          )}
      ...
      {showRefundModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#142314] border border-red-500/40 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4 text-left">
            <div className="w-12 h-12 rounded-2xl bg-red-950/80 border border-red-500/50 flex items-center justify-center text-red-400">
              <RotateCcw className="w-6 h-6" />
            </div>

            <h3 className="text-lg font-bold font-display text-white">
              Confirm 48-Hour Refund Request?
            </h3>

            <p className="text-xs text-emerald-200/80 leading-relaxed">
              By confirming, you will receive a full 100% refund of your 1st payment. Your account will immediately be downgraded to the free tier.
            </p>

            <div className="bg-[#0f1b0f] p-3 rounded-xl border border-[#203620] text-[11px] text-emerald-300/70">
              The refund will be credited back within 1 to 3 business days to your original PayPal or Card payment source.
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              ...
              <button
                ...
                disabled={actionLoading}
                className="px-5 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-bold text-xs shadow-lg flex items-center gap-1.5 cursor-pointer"
              >
